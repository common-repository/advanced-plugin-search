...
/**
 * Access via Klick_Aps()->db_operations().
 */
class Klick_Aps_Options {
	...
	public function get_option($option, $setting = false) {
		return get_option('klick-aps-' . $option, $setting);
	}
	...
	public function update_option($option, $setting = false) {
		return update_option('klick-aps-' . $option, $setting);
	}
	...
	public function delete_option($option) {
		delete_option('klick-aps-' . $option);
	}
	...
	public function get_option_keys() {
		return apply_filters(
			'klick_aps_option_keys',
			array('logging','notice-display-time','data-loaded','total-pages','affected-total-rows','advanced-search-toggle','advanced-search-notice')
		);
	}
	...
	public function save_settings($settings) {
		($settings['aps_advance_search_toggle'] == "ON" ? $this->update_option('advanced-search-toggle', true) : $this->update_option('advanced-search-toggle', false) );
		$return_array['messages'] = $this->show_admin_warning(__("Setting Saved.", "klick-aps"),'updated fade');
	...
	public function dismiss_page_notice_until($notice_id) {
		...
		$notices = Klick_Aps()->get_notifier()->get_notices();
	...
	public function admin_page_url() {
		return admin_url('admin.php?page=klick-aps');
	}
}
